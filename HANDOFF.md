...
## ▶ ACTIVE THREAD 2026-06-22 (b) — KK-MasterVP: VP-length re-sweep + FVG-anchored SL (commit `ade13a9`)
...
2. **✅ FVG-anchored SL IMPLEMENTED (engine, default OFF, committed `ade13a9`).** `kk::apply_fvg_sl`
   (`cpp_core/include/kk/mastervp/fvg_sl.hpp`) re-anchors a breakout stop just beyond the nearest
   significant 3-bar FVG between entry and the value edge, recomputes risk+TP1/TP2. `make test` green
   (+3 FVG tests). Keys `InpEnableFvgSl/FvgMode{0rep,1widen,2tighten}/FvgBeyondVa/FvgMinAtr/FvgBufAtr/
   FvgMinRiskAtr/FvgMaxRiskAtr/FvgLookback/FvgBreakoutOnly`. **Early signal:** loose cap (6 ATR) HURTS
   XAU-M3 OOS (stop balloons when price breaks far beyond VAL); tight caps are the lever to tune.
3. **⏳ IN FLIGHT:** FVG grid sweep (`vp_length_sweep_2026-06-22.py --fvg {XAU-M3,BTC-M3,BTC-M5}`,
   out `research/mastervp_parity/_fvg_sweep.out`, bg task `beas1q974`) — 9 configs × train+OOS × 3 targets.
   **NEXT:** read results → does FVG resurrect BTC-M3 / improve XAU-M3 OOS? If a config wins on BOTH
   windows → refine + overfitting gate; if it wins → port `apply_fvg_sl` into the MasterVP MQL5 EA for an
   MT5 A/B. If nothing helps → document FVG as tested-inert and keep locks.
4. **RECORDED (not yet tested):** user assumption that mean-reversion should fade LOCAL VP, not master
   (`[[reversion-local-vp-assumption]]`; code currently fades master). Plan in BUILD-PLAN.
