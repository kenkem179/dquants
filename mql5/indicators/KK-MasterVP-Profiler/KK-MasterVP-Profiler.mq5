...
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, KenKem"
...
input bool   InpSetShowRejects = true;  // Mark edge-cross triggers that were REJECTED, with the reason (pos open / chase / weak net / EMA opp)
...
input int    InpVpLookback   = 50;    // Local window (bars) - master window = this x master multiplier
input int    InpVpBins       = 40;    // Node bins - price buckets for POC/VAH/VAL + node-state math
input int    InpHistBins     = 120;    // Display bins - resolution of the drawn histogram rows
input int    InpMasterMult   = 3;     // Master multiplier - master window = local window x this
input bool   InpUseRealTicks = true;  // ON: bin the real broker tick stream (volume/delta/dwell at price); OFF: bar tick_volume at hlc3 (Pine parity)
...
string TagText(const NodeState &ns) {
   int pct = (int)MathRound((ns.absorbed ? (1.0 - MathAbs(ns.net)) : MathAbs(ns.net)) * 100.0);
   if(ns.absorbed)   return "~" + IntegerToString(pct) + "%";
   if(ns.state > 0)  return SymUp() + IntegerToString(pct) + "%";
   if(ns.state < 0)  return SymDn() + IntegerToString(pct) + "%";
   return "flat";
}
...
int OnInit() {
   ...
   g_histBins  = ClampI(InpHistBins, 4, 200);
...
void ComputeProjection(double close1, double atr1) {
   ...
   double nearDelta = g_tickFeed ? g_netChartTick : g_netChartNode;
...
void DrawHistogram(datetime tHistL, datetime tHistR) {
   ...
   for(int b = 0; b < g_histBins; b++) {
      ...
      if(tot > 0.0) netRef = MathMax(netRef, MathAbs(bv - sv) / tot);
   }
...
void DrawPanel(double atr1) {
   ...
   int rowH = 16, pad = 6, w = 184;
   int rows = InpShowExecRow ? 7 : 6;
   ...
   Kill(OBJPFX "tblBg"); Kill(OBJPFX "tblEx");
   for(int i = 0; i < 6; i++) Kill(OBJPFX "tbl" + IntegerToString(i));
   PanelBg(OBJPFX "tblBg", x, y - 3, w, rows * rowH + pad * 2);
   int tx = gutter + 12, ty = y + pad - 2;

   double close1 = iClose(_Symbol, g_tf, 1);
   double atrPct = (close1 > 0.0 && atr1 > 0.0) ? atr1 / close1 * 100.0 : 0.0;
   Lbl(OBJPFX "tbl0", tx, ty, StringFormat("KK-VP %s [%s]", TfShort(),
       g_tickFeed ? "TICK" : "BAR"), g_tickFeed ? clrDeepSkyBlue : clrOrange);
   ty += rowH;
   Lbl(OBJPFX "tbl1", tx, ty, StringFormat("net M1 %+d%% %s %+d%%",
       (int)MathRound((g_hasM1 ? g_netM1 : 0.0) * 100.0), TfShort(),
       (int)MathRound(g_netChartTick * 100.0)), clrWhite);
   ty += rowH;
   Lbl(OBJPFX "tbl2", tx, ty, StringFormat("net M5 %+d%% M15 %+d%%",
       (int)MathRound((g_hasM5 ? g_netM5 : 0.0) * 100.0),
       (int)MathRound((g_hasM15 ? g_netM15 : 0.0) * 100.0)), clrWhite);
   ty += rowH;
   Lbl(OBJPFX "tbl3", tx, ty, StringFormat("ATR%% %.3f slp %s", atrPct,
       g_slopeKnown ? StringFormat("%+.2f", g_slopeNorm) : "n/a"), clrSilver);
   ty += rowH;
   string stab = "n/a";
   ...
   Lbl(OBJPFX "tbl4", tx, ty, "POC " + stab, stabC);
   ty += rowH;
   // Execution-health slot: the label is created here (after the backdrop,
   // so the z-order holds) and then repainted per tick by UpdateExecRow.
   g_execRowX = tx; g_execRowY = -1;
   if(InpShowExecRow) {
      g_execRowY = ty;
      Lbl(OBJPFX "tblEx", tx, ty, "spr n/a  slip n/a", clrSilver);
      UpdateExecRow();
      ty += rowH;
   }
   // Plain-language headline: arrow + direction word + the magnet PRICE the
   // bias engine projects to - "where is price likely to go" instead of the
   // engine's internal label. TREND only when the master-POC slope agrees;
...
void UpdateVerdictAndRuler(bool force) {
   ...
   for(int b = 0; b < g_histBins; b++) {
      ...
      double sv = g_binSell[b] + g_liveSell[b];
      nearVol += bv + sv;
      if(bpx <= px) { if(bv > sv) tBu += (bv - sv); else tSu += (sv - bv); }
      else          { if(bv > sv) tBo += (bv - sv); else tSo += (sv - bv); }
   }
   ...
   string txt = (shown == 0) ? "Net Vol ~" + mag + "  " + cd
                : "Net Vol " + (shown > 0 ? SymUp() : SymDn()) + " " +
