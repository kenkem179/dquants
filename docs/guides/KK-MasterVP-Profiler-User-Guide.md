# KenKem Master Volume Profiler — User Guide

A practical, beginner‑friendly guide to reading the&nbsp;KenKem Master Volume Profiler&nbsp;indicator on MetaTrader 5, well tested with major pairs like XAUUSD and BTCUSD on M3, M5, M15.
...
- **What it is** — a display‑only chart indicator for reading market structure and recent flow.
- **Best default setup** — **XAUUSD on M5**.
- **What it shows** — profiles, a telemetry panel, a near‑price verdict tag, EMA context, and optional historical setup markers.
- **What it does not do** — place, modify, or close trades.
- **Best way to read it** — start with structure, then flow, then trend, then conditions.

## 1. What Is The Profiler
...
It never opens, modifies, or closes a position. Think of it as a map, not a steering wheel.

## 2. Before you start

The Profiler is built to be a visual twin of the KenKem Master Volume Profiler study.

### What to expect

- **Default tuning** — XAUUSD (Gold) on the M5 (5‑minute) timeframe.
- **Other symbols/timeframes** — it will still run, but the defaults are not tuned for them, so treat what you see as a rough sketch rather than a finished picture.
- **Safety** — it is display‑only. Attaching it cannot place a trade or change your account in any way.

## 3. Installing and attaching

1. **Copy** `KK-MasterVP-Profiler.ex5` into your MetaTrader 5 `MQL5/Indicators/` folder, or the subfolder where you keep custom indicators in.
2. **Restart MetaTrader 5**, or right‑click the **Navigator → Indicators** list and choose **Refresh**.
3. **Open** an **XAUUSD, M5** chart.
4. **Attach** `KK‑MasterVP‑Profiler` from the Navigator onto the chart, or double‑click it.
5. In the settings window, **leave the defaults** as they are for the intended setup, then click **OK**.
...
- **Side histogram** — horizontal bars showing where activity has concentrated. Longer bar = more activity at that price. A brighter colored slice hints at the recent buy/sell lean.
- **mPOC / mVAH / mVAL** — the three master profile lines: the single busiest price (POC) and the upper/lower edges of its core activity zone (VAH/VAL).
- **lPOC / lVAH / lVAL** — the same three lines for the local (recent) profile. Drawn faint and labeled as secondary context.
- **pPOC** — a predicted version of the busiest price — a preview of where the center of activity may drift next.
- **Net / over / under tag** — the near‑price verdict sitting next to live price: who has been more active right around the current price.
- **Telemetry panel** — a compact card (top‑right by default) with the live readouts described in §6.
- **EMA overlay** — four moving‑average lines plus an optional shaded ribbon, for trend context.
- **Setup markers** — optional E / SL / TP1 / TP2 lines with a WON / LOST / BE tag, showing how past setups resolved.
- **Guide lines** — thin reference lines a fixed distance above and below live price, as a visual ruler.
...
### A calm way to read them

- **Start with the master lines** — they tell you the important shelves and ceilings.
- **Then check the local lines and live price** — they tell you where you are relative to that bigger structure.
- **Keep it simple** — no urgency, no prediction required.
...
- **A long run of WON tags is not a guarantee** — markets change, and historical illustration is not future performance.
- **The risk‑percentage figure is display-only** — it is not an instruction to risk that amount, and your broker’s own minimums, steps, and limits apply.
- **Rejection markers are explanatory** — they point out spots where a setup almost formed but was filtered out, with a short reason such as EMA opp (“the flow leaned one way but the trend disagreed”) or a chase note. These are there to help you understand the logic, nothing more.
- **Break-even ratchet markers are visual only** — they only change how the historical markers are drawn and never affect anything live.
...
- **Also included in this group** — how far back to scan, how many markers to keep, the two target distances (TP1/TP2 in R), and a display‑only example‑risk figure.

### Visuals

- **Volume-profile window length**
- **Master lines on/off switch**
- **Local lines on/off switch**
- **Histogram on/off switch**
- **Histogram front/behind candles toggle**

These controls let you keep only what you find useful.

### EMA Overlay

- **Show EMAs**
- **Zone-ribbon switch**
- **Three editable lengths** — fast / medium / slow — if you like to customize the look.
...
1. **Start with structure** — note where price sits relative to the master POC and the value‑area edges. Inside the band, between edges, or pushing beyond one?
2. **Add the flow** — glance at the panel’s net reads and the near‑price verdict. Do buyers or sellers appear more active, and does that agree with the structure?
3. **Sanity-check the trend** — let the EMA overlay tell you whether the broader backdrop agrees or disagrees.
4. **Mind the conditions** — if the execution‑health row is flashing `!` or `!!`, conditions are rougher than usual — a reason to be patient, not hurried.
5. **Use the history as study, not as a promise** — the WON/LOST markers are there to build your understanding of the style, not to chase the next one.
...
- **R** means “risk units” — a multiple of the distance from entry to the stop reference (so 0.8R is a target eight‑tenths of that distance away).
- **ATR** is a standard measure of how much price typically moves; several distances are expressed as a multiple of it so the drawing stays sensible whether the market is quiet or busy.
...
**Always shown:** the telemetry panel, the near‑price verdict tag, the predicted‑POC line, and the spread/speed (execution‑health) row.

A number of deeper fine‑tuning values are kept internal on purpose, so this list stays short and the chart stays easy to read. You don’t need any of them to use the Profiler well.
