...
CNodeEngine   g_node;
int  hAtr,hRsi,hAdx,hEmaF,hEmaS,hHtfEmaF,hHtfEmaS,hAtrM1;
...
int OnInit()
{
   ...
   hAtrM1=iATR(_Symbol,PERIOD_M1,InpAtrLen);   // Monster: M1 near-price-net window ATR
...
void OnNewBar()
{
   ...
   // ----- Monster: the impulse path REPLACES the base entry ABOVE the vol ceiling -----
   ...
   if(InpEnableImpulse && aboveCeil){
      ...
      bool slopeUp=false, slopeDn=false;                        // master-POC slope
      VPResult mPrev;
   ...
   double atrPct=(price>0)?AtrAt(1)/price*100.0:0.0;                        // ATR% band
   if(!isImpulse){                                                          // impulse owns the ABOVE-ceiling band
